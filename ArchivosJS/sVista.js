document.getElementById("btn-1").addEventListener("click", function() {
    // Coloca la URL de la otra vista o página web que deseas abrir en el siguiente enlace:
    window.location.href = "retiro.html"; // Reemplaza "otra-vista.html" con la URL deseada
});

document.getElementById("btn-3").addEventListener("click", function() {
    // Coloca la URL de la otra vista o página web que deseas abrir en el siguiente enlace:
    window.location.href = "cambio_clave.html"; // Reemplaza "otra-vista.html" con la URL deseada
});

document.getElementById("btn-5").addEventListener("click", function() {
    // Coloca la URL de la otra vista o página web que deseas abrir en el siguiente enlace:
    window.location.href = "../index.html"; // Reemplaza "otra-vista.html" con la URL deseada
});

document.getElementById("btn-2").addEventListener("click", function() {
    window.location.href = "clave.html"; 
});
